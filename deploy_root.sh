...
echo "4. Deploying to ROOT path (Critical Fix)..."
expect -c '
set timeout 60
spawn ssh -o StrictHostKeyChecking=no root@148.230.88.203 "cd /var/www/coa-viewer && tar -xzf dist.tar.gz && cp -r dist/* . && rm -rf dist && rm dist.tar.gz && systemctl restart nginx"
expect "password:"
send "Mv+7c#dQ4U9ALV4Lup#p\r"
expect eof
'
