...
module.exports = {
    report: (analysis, options = {}) => {
        const { summary, ghosts, zombies, ignored } = analysis;
        ...
        if (!options.json) {
            ...
            const verdict = ghosts.length > 0 ? 'FAIL' : 'PASS';
            const color = ghosts.length > 0 ? colors.red : (zombies.length > 0 ? colors.yellow : colors.green);
        ...
        if (options.markdown) {
            ...
            md += `**Verdict**: ${ghosts.length > 0 ? '🔴 FAIL' : (zombies.length > 0 ? '🟡 PASS (with Warnings)' : '🟢 PASS')}\n\n`;
            md += `| Metric | Count |\n| :--- | :---: |\n`;
            ...
            console.log(md);
            return analysis; // Return analysis data just in case
        }
