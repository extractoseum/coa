...
import {
    ...
    FileBox, Activity, Wrench, MessageCircle, Archive, Zap, X, Package, Code, Loader2, ExternalLink
} from 'lucide-react';
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-white/[0.02] p-4 rounded-3xl border border-white/5">
                                        ...
                                        <div className="flex flex-wrap gap-1">
                                            {/* Render Contact Tags (Badges) */}
                                            {selectedConv.tags && selectedConv.tags.length > 0 ? (
                                                selectedConv.tags.map(tag => (
                                                    <span key={tag} className="px-2 py-0.5 bg-yellow-500/10 text-yellow-500 text-[9px] rounded-md font-bold border border-yellow-500/20">{tag}</span>
                                                ))
                                            ) : (
                                                <span className="text-[9px] opacity-30 italic">Sin etiquetas</span>
                                            )}
                                        </div>
                                    </div>
                                    <div className="bg-white/[0.02] p-4 rounded-3xl border border-white/5">
                                ...
                                <div className="space-y-2">
                                    {selectedConv.facts?.action_plan && selectedConv.facts.action_plan.length > 0 ? (
                                        selectedConv.facts.action_plan.map((action: any, j: number) => (
                                            <button key={j} className="w-full text-left p-4 rounded-2xl bg-black/20 border border-white/5 group hover:border-pink-500/40 transition-all flex items-center justify-between">
